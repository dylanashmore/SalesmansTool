...
type ElectricCategory = "sedan" | "truck" | "sports" | "hatchback" | "suv" | "minivan";

const CATEGORY_ORDER: ElectricCategory[] = ["sedan", "truck", "sports", "hatchback", "suv", "minivan"];

const CATEGORY_LABEL: Record<ElectricCategory, string> = {
  sedan: "Sedan",
  truck: "Truck",
  sports: "Sport",
  hatchback: "Hatch",
...
export default function ElectricScreen() {
  ...
  const electricVehicles = useMemo(() => vehicles.filter((v) => v.type === "electric"), []);
  ...
  const cardsByCategory = useMemo(() => {
    const grouped: Record<ElectricCategory, typeof modelCards> = {
      sedan: [],
      truck: [],
      sports: [],
      hatchback: [],
    ...
    for (const card of modelCards) {
      const cat = card.category as ElectricCategory;
  ...
  const headerIndexByCategory = useMemo(() => {
    const m = new Map<ElectricCategory, number>();
  ...
  const [activeCat, setActiveCat] = useState<ElectricCategory>(availableCats[0] ?? "suv");

  const sortedHeaderIndices = useMemo(() => {
    const arr: Array<{ cat: ElectricCategory; idx: number }> = [];
    headerIndexByCategory.forEach((idx, cat) => arr.push({ cat, idx }));
    return arr.sort((a, b) => a.idx - b.idx);
  }, [headerIndexByCategory]);

  const categoryForListIndex = (i: number): ElectricCategory | null => {
    let best: { cat: ElectricCategory; idx: number } | null = null;
  ...
  const jumpTo = (cat: ElectricCategory) => {
    ...
    <View style={{ flex: 1, backgroundColor: "#1C1C1E" }}>
      ...
      <View style={{ paddingTop: 60, paddingBottom: 20, paddingHorizontal: 22 }}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <Text style={{ color: "#FFF", fontSize: 22, fontWeight: "800", letterSpacing: -0.4, flex: 1 }}>Electric Vehicles</Text>
          <SearchButton />
        </View>
              ...
              <Pressable
                onPress={() =>
                  router.push({
                    ...
                    params: { model: item.model, type: "electric" },
                  })
                }
                style={({ pressed }) => ({
                  backgroundColor: "#FFF",
